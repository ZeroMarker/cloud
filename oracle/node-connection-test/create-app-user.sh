...
app_user="${1:-${APP_DB_USER:-APP_USER}}"

if [[ ! "$app_user" =~ ^[A-Za-z][A-Za-z0-9_]{0,29}$ ]]; then
  printf 'Invalid application username: %s\n' "$app_user" >&2
  exit 2
fi
...
cleanup() {
  unset DB_USER DB_PASSWORD DB_WALLET_PASSWORD APP_DB_USER APP_DB_PASSWORD
  unset DB_DSN DB_WALLET_DIR
}
trap cleanup EXIT
...
if [[ -z "${DB_PASSWORD:-}" ]]; then
  read -r -s -p 'ADMIN database password: ' DB_PASSWORD
  printf '\n'
fi
export DB_PASSWORD
...
if [[ -z "${APP_DB_PASSWORD:-}" ]]; then
  read -r -s -p "New password for $APP_DB_USER: " APP_DB_PASSWORD
  printf '\n'
fi
